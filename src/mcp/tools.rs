...
fn build_tools_raw() -> Vec<ToolDefinition> {
    vec![
        ...
        tool(
            "rust_analyzer_workspace_diagnostics",
            "Get all compiler diagnostics across the entire workspace. Files are paginated (default 50 files per page); the response includes { workspace, files, summary, pagination: { total_files, returned_files, next_cursor? } }. Use cursor for next page, limit to override, or verbose=true for all files (capped at 1000). The `summary` totals always cover the whole workspace, not just the page. Files this server has already opened are re-synced from disk before every tool call, so edits made outside rust-analyzer (Edit tool, git operations, format-on-save in another editor) are reflected. Coverage is still limited to what rust-analyzer has analysed: a file it has never looked at contributes nothing here, so `cargo check` remains the exhaustive answer for a cold workspace.",
            json!({
                "type": "object",
                "properties": {
